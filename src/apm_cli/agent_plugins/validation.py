...
def validate_lsp_extension_file(
    path: Path,
    *,
    isolate_invalid_servers: bool = False,
    plugin_root: Path | None = None,
    ...
    return validate_lsp_extension_document(
        read_json_document(path),
        isolate_invalid_servers=isolate_invalid_servers,
        plugin_root=plugin_root if plugin_root is not None else path.parent.parent,
    )


def validate_lsp_extension_document(
    document: object,
    *,
    isolate_invalid_servers: bool = False,
    plugin_root: Path | None = None,
    ...
    for name, config in servers.items():
        ...
        if not isinstance(name, str) or not isinstance(config, dict):
            server_errors.append("Every LSP server must have a string name and object config")
            _record_lsp_server_errors(
                result,
                server_errors,
                isolate_invalid_servers=isolate_invalid_servers,
            )
            continue
        server_errors.extend(_lsp_server_shape_errors(name, config))
        if server_errors:
            _record_lsp_server_errors(
                result,
                server_errors,
        ...
        try:
            ...
            server_errors.append(f"lspServers.{name} is invalid: {exc}")
            _record_lsp_server_errors(
                result,
                server_errors,
                isolate_invalid_servers=isolate_invalid_servers,
        ...
        if not (_BARE_COMMAND_RE.fullmatch(command) or command.startswith("./")):
            server_errors.append(f"lspServers.{name}.command must be a bare token or a ./ path")
        elif command.startswith("./") and _plugin_relative_path_escapes(
            command[2:],
            plugin_root,
        ):
            server_errors.append(f"lspServers.{name}.command escapes the plugin root")
    ...
    if result.errors and not isolate_invalid_servers:
        return result
    result.normalized = {"lspServers": normalized}
    return result


def _record_lsp_server_errors(
    result: ValidationResult,
    errors: list[str],
    *,
    isolate_invalid_servers: bool,
) -> None:
    if isolate_invalid_servers:
        result.warnings.extend(f"{error}; server ignored" for error in errors)
    else:
        result.errors.extend(errors)


def _lsp_server_shape_errors(name: str, config: dict[str, Any]) -> list[str]:
    errors = [
        f"lspServers.{name} contains an unknown field: {field}"
        for field in sorted(set(config) - _LSP_SERVER_FIELDS)
    ]
    args = config.get("args")
    if args is not None and (
        not isinstance(args, list) or not all(isinstance(item, str) for item in args)
    ):
        errors.append(f"lspServers.{name}.args must be an array of strings")
    env = config.get("env")
    if env is not None:
        if not isinstance(env, dict) or not all(
            isinstance(key, str) and isinstance(value, str) for key, value in env.items()
        ):
            errors.append(f"lspServers.{name}.env must map strings to strings")
        elif len({key.casefold() for key in env}) != len(env):
            errors.append(f"lspServers.{name}.env contains a duplicate case-insensitive name")
    for field_name in ("startupTimeout", "shutdownTimeout", "maxRestarts"):
        value = config.get(field_name)
        if value is not None and (
            isinstance(value, bool) or not isinstance(value, int) or value < 0
        ):
            errors.append(f"lspServers.{name}.{field_name} must be a non-negative integer")
    restart_on_crash = config.get("restartOnCrash")
    if restart_on_crash is not None and not isinstance(restart_on_crash, bool):
        errors.append(f"lspServers.{name}.restartOnCrash must be a boolean")
    return errors
