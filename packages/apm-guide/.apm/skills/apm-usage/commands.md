...
|---------|---------|-----------|
| `apm init [NAME]` | Initialize a new APM project; names must be non-empty and must not contain path separators or equal `..`. A derived filesystem-root name falls back to `my-project`. | `-y` skip prompts, `--target` comma-separated targets (CLI aliases such as `agents` and `vscode` are persisted as canonical `copilot`). `apm init --plugin` is deprecated; prefer `apm plugin init`. Use `apm plugin init --claude-plugin` only for the historical Claude-compatible scaffold. `--marketplace` seeds apm.yml with a `marketplace:` block. |
...
| Command | Purpose | Key flags |
|---------|---------|-----------|
| `apm pack` | Build distributable artifacts from `apm.yml`. A `dependencies:` mapping, including `{}`, produces the default Agent Plugin v1 bundle. `plugin.json` is the artifact manifest; the root `apm.lock.yaml` records resolved provenance and per-file integrity. Production MCP servers come from resolved lock state and must be representable by Agent Plugins v1. | `--plugin` explicit Agent Plugin output, `--claude-plugin` historical Claude output, `--format apm` legacy APM output, `--archive`, `--archive-format [zip\|tar.gz]`, `--dry-run`, `--force`, `--offline`, `--json`, `--check-versions`, `--check-clean`. |
