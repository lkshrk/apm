...
One `apm.yml`. Ten default harnesses. Reproducible AI agent setup. Every developer who clones the repo runs `apm install` and gets the same supported primitives wired into Copilot, Claude, Grok Build, Cursor, OpenCode, Codex, Gemini, Windsurf, Kiro, and Hermes. Antigravity is available as an explicit CLI target. The lockfile pins exact versions and content hashes.
