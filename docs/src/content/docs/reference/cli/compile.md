...
`antigravity` is explicit-only, while `intellij` is MCP-only.
Neither is included in `all`. Hermes is included; use `--single-agents` when
compiling for Hermes because it reads only the top-level `AGENTS.md`.
